...
var HTMLonlineURL = '<div class=online-class><a href="%data%" class="url" target="_blank">%data%</a></div><br>';
...
// load a map and add the markers
...
function initializeMap() {
  ...
  function createMapMarker(placeData) {
    ...
    var description = locationsDescription[name.split(", ")[0]];
    ...
    // create an info window of the marker
    var infoWindow = new google.maps.InfoWindow({
      content: '<h4>' + name + '</h4>'+
               '<p>' + description + '</p>'
    });

    // add an event opening the infoWindow
    google.maps.event.addListener(marker, 'click', function() {
      infoWindow.open(map,marker);
    });

    // adjust the map borders according to the added pins
